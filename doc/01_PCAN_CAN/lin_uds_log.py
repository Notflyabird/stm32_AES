...
LIN UDS Logger
==============
Generates a structured log file for each test run.
Log file: Result/SVVR_<scriptname>_<timestamp>.log

Format per entry:
  [HH:MM:SS.mmm] [TX/RX/INFO] <description>  [PASS/FAIL]
"""
...
from lin_tp_config import LOG_DIR


# --------------------------------------------------------------------------
# ANSI colours (console only, stripped in file)
# --------------------------------------------------------------------------
_GREEN  = "\033[92m"
_RED    = "\033[91m"
_YELLOW = "\033[93m"
...
class LinUdsLog:
    """
    Logger for LIN UDS service scripts.
    ...
    Usage:
        ...
        log.tx("10 02")
        log.rx("50 02")
        log.result(True, expected="50 02", received="50 02")
        log.close()
    """
    ...
    def frame(self, direction: str, hex_bytes: str):
        self._logger.info(f"[{direction:<6}] {hex_bytes}")
...
# --------------------------------------------------------------------------
# Helper used by all service scripts
# --------------------------------------------------------------------------
...
def read_current_session(tp, log: LinUdsLog) -> bytes:
    """
    Send 22 F1 86 (ReadDataByIdentifier – ActiveDiagnosticSession) and log result.
    Call this at the start of every test script to record the initial session.
...
def check_negative_response(response: bytes, sid: int, nrc: int, log: LinUdsLog,
                             step_desc: str = "") -> bool:
    """
...
def check_no_response(response: bytes, log: LinUdsLog, step_desc: str = "") -> bool:
    """
...
def check_positive_response(response: bytes, sid: int, log: LinUdsLog,
                             step_desc: str = "") -> bool:
    """
